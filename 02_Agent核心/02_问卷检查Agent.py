...
# 对照 llm-agent：question_check_agent.py 里的 SYSTEM_PROMPT
# 好的 Prompt 有三个特点：
#   1. 明确告诉 AI 它的角色和任务
#   2. 列清楚所有分类和关键词
#   3. 强制要求返回固定 JSON 格式
...
# 对照 llm-agent：question_check_agent.py 里的 _keyword_fallback 方法
...
class QuestionCheckAgent(BaseAgent):
    ...
    对照 llm-agent：这就是 question_check_agent.py 的精简版。
    """
    ...
    def process(self, input_json: Dict[str, Any]) -> AgentResult:
        ...
        对照 llm-agent：question_check_agent.py 的 process() 方法
        """
    ...
    def _check_single(self, question: str, answer_text: str) -> dict:
        ...
        对照 llm-agent：question_check_agent.py 里的 _check_single 方法
        """
        # 先试 LLM
        try:
    ...
    def _llm_check(self, question: str, answer_text: str) -> dict:
        """
        调用 LLM 进行分析。
        对照 llm-agent：question_check_agent.py 里的 _llm_check 方法
        """
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print("✅ 你刚刚写了一个完整的 Agent！对照 llm-agent/question_check_agent.py 看看，")
